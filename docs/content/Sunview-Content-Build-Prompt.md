...
Write one page at a time, in this order, and stop after each for approval. For every page deliver: URL, meta title (≤60 chars), meta description (≤155), H1, eyebrow label (rendered as <span>, not a heading), body sections with H2/H3 (strict hierarchy, no skipped levels), CTAs (link every CTA to `/admissions/`), FAQ block (3–5 questions) and internal links to at least 3 sibling pages.
